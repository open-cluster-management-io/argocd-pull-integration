...
.PHONY: clean
clean: ## Remove build artifacts and tool binaries (handles read-only envtest binaries).
	@if [ -n "$(LOCALBIN)" ] && [ "$(LOCALBIN)" != "/" ] && [ -d "$(LOCALBIN)" ]; then \
		case "$(LOCALBIN)" in "$(CURDIR)"/*) ;; *) echo "Refusing to clean LOCALBIN outside repo: $(LOCALBIN)"; exit 1;; esac; \
...
test-e2e-advanced-pull-custom-namespace: manifests generate fmt vet ## Run advanced pull e2e test with custom ArgoCD namespaces
	...
	helm install argocd-agent-addon \
		...
		--set gitOpsCluster.customArgoCDCR.enabled=true \
		--set image=quay.io/open-cluster-management/argocd-pull-integration \
		--set tag=latest \
		--set principalServiceType=NodePort \
		--wait \
